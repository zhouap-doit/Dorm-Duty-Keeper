...
const submitApply = async () => {
  ...
  try {
    ...
    // 1. 模糊查询宿舍（去除前后空格，忽略大小写）
    const searchName = dormitoryName.value.trim()
    ...
    if (dormError || !dorm) {
      console.error('查询宿舍失败:', dormError)
      uni.showToast({ 
        title: `未找到宿舍"${searchName}"，请检查名称`, 
        icon: 'none',
        duration: 3000
      })
      return
    ...
    if (existingApp) {
      if (existingApp.status === 'pending') {
        uni.showToast({ title: '您已提交过申请，请等待审核', icon: 'none' })
        return
      }
      if (existingApp.status === 'approved') {
        uni.showToast({ title: '您已是该宿舍成员', icon: 'none' })
        return
      }
    }
    ...
    const { error: applyError } = await supabase
      ...
      .insert({
        ...
        user_id: user.id,
        applicant_name: memberName.value.trim(),
        status: 'pending'
      })

    if (applyError) {
      console.error('提交申请失败:', applyError)
      throw applyError
    }
    
    console.log('申请已成功提交，宿舍ID:', dorm.id)
