...
// 自动排班区域：填充与排序控制
async function fillAutoMembersFromDorm() {
  const list = await getDormMembers()
...
// 手动排班区域：填充宿舍成员与顺序调整
async function fillManualMembersFromDorm() {
  const list = await getDormMembers()
  if (!list.length) { uni.showToast({ title:'暂无宿舍成员', icon:'none' }); return }
  manualMembers.value = [...list]
