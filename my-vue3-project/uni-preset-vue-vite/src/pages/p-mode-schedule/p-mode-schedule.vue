...
const publish = () => {
  ...
  try {
    const dutyData = {
      ...
      // P人模式的数据结构
      schedule: [], // P人模式不需要预设值日安排
      checkins: [], // 打卡记录，初始为空
      members: ['张三', '李四', '王五'], // 宿舍成员列表
      cycleStatus: 'active' // 循环状态：active(进行中), completed(已完成)
    }
    uni.setStorageSync('duty_schedule_data', dutyData)
    // 设置排班表数据用于预览
    scheduleData.value = dutyData
    
    // 【重要】设置值日模式为P，并触发全局事件通知成员页面
    uni.setStorageSync('duty_mode', 'P')
    uni.$emit('dutyScheduleStorageUpdated', dutyData)
    console.log('[P人模式] 已发送值班表更新事件给成员页面')
  } catch (e) {
