...
// 响应式数据
const dormitoryName = ref('301宿舍')
const managerName = ref('张三')
const dutyMode = ref('J人模式')
...
const loadData = () => {
  try {
    ...
    if (dormitoryInfo) {
      dormitoryName.value = dormitoryInfo.dormitoryName || '301宿舍'
    }
    
    const managerProfile = uni.getStorageSync('manager_profile')
    if (managerProfile) {
      managerName.value = managerProfile.userName || '张三'
    }
